...
function stack_plot2(
    ...
    cmap = [vcat(zeros(size(idat[ipts, i])[1]), 0) for i in 1:ntraces]  # color map per point
    dt_seconds = mean(diff(tdat[:, 1]))
    # class = "direct"
    clmap = Dict(
        "direct" => 4, "evoked" => 6, "spontaneous" => 2, "noise" => 1, "artifact" => 9
    )  # for use with paired_10
    for (class, clevel) in clmap
        for i in 1:ntraces
            ...
            amp = sign .* evk[(evk.peaktime .< tmax * 1e3), :amp] .*1e-12
    ...
    stim_lats = 1e-3 .* Acq4Reader.get_stim_arg("latency", data_info; device="Laser")
    stx = [vcat([stim_lats[i], stim_lats[i]], NaN) for i in 1:size(stim_lats)[1]]
